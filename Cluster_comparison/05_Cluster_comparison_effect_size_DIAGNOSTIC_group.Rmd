...
run_for_all_clusters <- function(df, group_col = "group", cols, out_prefix) {
...
If we want to only compare certain diagnostic groups (pair0wise comparisons)
```{r}
compute_tstats_pairwise <- function(df_a, df_b) {
  stopifnot(identical(colnames(df_a), colnames(df_b)))
  cols <- colnames(df_a)

  safett <- function(x, y) {
    # Handle all-NA or zero-variance gracefully
  ...
  out <- lapply(cols, function(col) {
    tt <- safett(df_a[[col]], df_b[[col]])
    data.frame(
      label   = col,
      tstat   = unname(tt$statistic),
      pvalue  = tt$p.value,
      mean_a  = mean(df_a[[col]], na.rm = TRUE),
      mean_b  = mean(df_b[[col]], na.rm = TRUE),
      n_a     = sum(!is.na(df_a[[col]])),
      n_b     = sum(!is.na(df_b[[col]])),
...
run_pairwise_comparisons <- function(df,
                                     group_col = "group",
                                     cols,
                                     pairs = list(c("SSD", "Control"),
                                                  c("ASD", "Control")),
                                     out_prefix) {
  # sanity: all requested groups exist
  have <- unique(df[[group_col]])
  needed <- unique(unlist(pairs))
  missing <- setdiff(needed, have)
  if (length(missing)) {
    stop("These groups are missing in `", group_col, "`: ",
  ...
  for (pp in pairs) {
    ...
    df_a <- df %>%
      dplyr::filter(.data[[group_col]] == gA) %>%
      dplyr::select(all_of(cols))
    df_b <- df %>%
      dplyr::filter(.data[[group_col]] == gB) %>%
      dplyr::select(all_of(cols))

    res <- compute_tstats_pairwise(df_a, df_b)

    # Nice, explicit filename: e.g., C_RS_noGSR_pairwise_SSD_vs_Control.csv
    out <- sprintf("%s_%s_vs_%s_effectsize.csv",
                   out_prefix,
                   gA, gB)
...
Selecting Columsn per imaging modality 
```{r}
# Cortical thickness 
...
```{r}
outdir <- ".../SNF_output1/effect_size/diagnostic_groups/"
...
Computing t-stat for pairwise comparisons 
```{r}
outdir <- ".../SNF_output1/effect_size/diagnostic_groups_pairwise"
dir.create(outdir, showWarnings = FALSE, recursive = TRUE)

# CT
run_pairwise_comparisons(
  df = compare_df2,
  group_col = "group",                 # SSD / ASD / Control live here
  cols = ct_cols,
  pairs = list(c("SSD","Control"), c("ASD","Control")),
  out_prefix = file.path(outdir, "C_CT_pairwise")
)
...
run_pairwise_comparisons(
  ...
  cols = sa_cols,
  pairs = list(c("SSD","Control"), c("ASD","Control")),
  out_prefix = file.path(outdir, "C_SA_pairwise")
)
...
run_pairwise_comparisons(
  ...
  cols = vol_cols,
  pairs = list(c("SSD","Control"), c("ASD","Control")),
  out_prefix = file.path(outdir, "C_VOL_pairwise")
)
...
run_pairwise_comparisons(
  ...
  cols = EA_cols,
  pairs = list(c("SSD","Control"), c("ASD","Control")),
  out_prefix = file.path(outdir, "C_EA_pairwise")
)

#RS
run_pairwise_comparisons(
  df = compare_df3,
  group_col = "group",                 # uses diagnostic group factor
  cols = RS_cols,                      # your RowMean_* columns
  pairs = list(c("SSD","Control"), c("ASD","Control")),
  out_prefix = file.path(outdir, "C_RS_noGSR_pairwise")
)
...
ct <- read.csv(".../SNF_output1/effect_size/diagnostic_groups_pairwise/C_CT_pairwise_SSD_vs_Control_effectsize.csv") 
...
#Flipping the signs, TURN OFF FOR PAIRWISE 
#ct_plot <- ct_mapped %>% #for diagnostic comparisons, do not need to invert
#  mutate(tplot = -tstat)
#ct_plot <- ct_plot %>% dplyr::select(label, tplot)

#This one for pairwise comparisons 
ct_plot <- ct_mapped %>% #for diagnostic comparisons, do not need to invert
  mutate(tplot = tstat)
ct_plot <- ct_plot %>% dplyr::select(label, tplot)
...
ggseg(
  ...
  labs(title = "Cortical Thickness — SSD vs Control", fill = "T-stat")

```
Surface Area
```{r}
# Read your attached CSV
sa <- read.csv(".../SNF_output1/effect_size/diagnostic_groups_pairwise/C_SA_pairwise_SSD_vs_Control_effectsize.csv")  
...
#Flipping the signs 
sa_plot <- sa_mapped %>%
  mutate(tplot = tstat) #change sign back to negative
...
ggseg(
  ...
  scale_fill_gradient2(low = "blue", mid = "white", high = "red",
                       midpoint = 0, limits = c(-8, 10)) +
  labs(title = "Surface Area — SSD vs TDC", fill = "T-stat")
...
vol <- read.csv(".../SNF_output1/effect_size/diagnostic_groups_pairwise/C_VOL_pairwise_SSD_vs_Control_effectsize.csv")
...
vol_map <- vol_map %>% mutate(tstat = tstat) #flip back for cluster comparison
...
ggseg(atlas = ggseg::aseg,
  ...
  scale_fill_gradient2(low = "blue", mid = "white", high = "red",
                       midpoint = 0, limits = c(-8, 10)) +
  labs(title = "Subcortical Volume — SSD vs TDC", fill = "T-stat")
...
```{r}
# Read your attached CSV
ea <- read.csv("/projects/tsecara/metaSNF/SNF/full_sample_no_FA_MD/output7/effect_size/C_EA_C1_effectsize.csv")   # adjust path if needed
```
...
```{r}
# Read your attached CSV
rs <- read.csv(".../SNF_output1/effect_size/diagnostic_groups_pairwise/C_RS_noGSR_pairwise_SSD_vs_Control_effectsize.csv")  
...
# Flip sign
mat_df <- mat_df %>%
  mutate(tplot = tstat) #change back if you want a sign flip
...
ggplot(mat_df, aes(x = col_lab, y = row_lab, fill = tplot)) +
  ...
  scale_fill_gradient2(
    ...
    high = "red",
    midpoint = 0,
    limits = c(-10, 5), # was originally -4, 5
    na.value = "grey90"
  ) +
